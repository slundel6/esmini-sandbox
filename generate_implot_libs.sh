...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == cygwin* ]]; then
    ...
    cmake -G "${GENERATOR[@]}" ${GENERATOR_ARGUMENTS} -DCMAKE_BUILD_TYPE=Release .. -DCMAKE_CXX_FLAGS="-fPIC" -DCMAKE_CXX_STANDARD=17 -DCMAKE_OSX_ARCHITECTURES="$macos_arch"
    cmake --build . $PARALLEL_ARG

elif [[ "$OSTYPE" == "linux-gnu"* ]]; then

    cmake -G "${GENERATOR[@]}" ${GENERATOR_ARGUMENTS} -DCMAKE_BUILD_TYPE=Debug .. -DCMAKE_CXX_FLAGS="-fPIC" -DCMAKE_CXX_STANDARD=17
    cmake --build . $PARALLEL_ARG
    mv libimplot.a libimplotd.a

    rm CMakeCache.txt
    cmake -G "${GENERATOR[@]}" ${GENERATOR_ARGUMENTS} -DCMAKE_BUILD_TYPE=Release .. -DCMAKE_CXX_FLAGS="-fPIC" -DCMAKE_CXX_STANDARD=17
    cmake --build . $PARALLEL_ARG
